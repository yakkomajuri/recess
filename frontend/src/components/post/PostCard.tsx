...
const PostCardTitle = ({
  ...
  const unfollowFeed = async (e: Event, feedUuid: string) => {
    ...
    const newLocalUser = {
      ...localUser,
      feeds_following: ((localUser || user!).feeds_following || []).filter(
        (uuid: string) => uuid !== feedUuid
      ),
    };
      ...
      <Col>
        {localUser?.feeds_following.includes(feedUuid) ? (
          <Button
            type="primary"
            size="small"
            onClick={(e) => unfollowFeed(e as any, feedUuid)}
          >
            Following
          </Button>
        ) : (
          <Button
            type="dashed"
            size="small"
            onClick={(e) => followFeed(e as any, feedUuid)}
          >
            Follow
          </Button>
        )}
      ...
      actions={[
        ...
        >
          ...
          <span style={{ paddingLeft: 8 }}>
            {(updatedPost || post).post_like_count}
          </span>
        </div>,
        <div
        ...
        >
          <LinkOutlined />
          <span style={{ paddingLeft: 8 }}>View content</span>
        </span>,
      ]}
    >
      <Card.Meta
